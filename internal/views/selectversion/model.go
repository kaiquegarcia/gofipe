...
func newModel(versions entity.GetVersionsResponse) *model {
	options := make([]list.Item, len(versions)+1)
	for i, b := range versions {
		options[i] = item(b)
	}
	options[len(versions)] = item(entity.Version{Code: "", Name: "Voltar ao início"})
	list := list.New(
		options,
		itemDelegate{},
		30,
