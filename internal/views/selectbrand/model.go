...
func newModel(brands entity.GetBrandsResponse) *model {
	options := make([]list.Item, len(brands)+1)
	for i, b := range brands {
		options[i] = item(b)
	}
	options[len(brands)] = item(entity.Brand{Code: "", Name: "Voltar ao início"})
	list := list.New(
		options,
		itemDelegate{},
		30,
